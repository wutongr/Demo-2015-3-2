...
+ (NSString *)quickConvert:(NSString *)hzString {
    ...
    if (cache == NULL) {
        ...
        if (cacheCeil == 0 && cacheFloor == 0xffffffff) {
            ...
            for (int i = 0; i < pycount; i++) {
                ...
                NSString *hz = [NSString stringWithUTF8String:pycell.hanzi] ;
    ...
    for (int i = 0; i < hzLength; i++) {
        ...
        [string appendFormat:@"%@",[NSString stringWithUTF8String:pycell.pinyin]];
    }
   ...
   for (int j = 0; j < [hzString length]; j++)
   {
      ...
      const unsigned char *byte = [data bytes];
       ...
       if((0x10 <= bh) && (bh <= 0x57))//是gb2312汉字
       {
           ...
           {
              NSRange  tRang = [ [NSString stringWithUTF8String:pinyinTable[j].hanzi] rangeOfString:[hzString substringWithRange:rang]];
              
               if (tRang.location != NSNotFound)
               {
                   pyString = [pyString stringByAppendingFormat:@"%@",[NSString stringWithUTF8String:pinyinTable[j].pinyin]];
